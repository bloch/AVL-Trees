/*
 *  AVL PROJECT - Data Structure Course, Tel Aviv University
 * 
 *  By :
 * 		Shay Fux & Nathan Bloch (TAU)
 *		
 */
...
public class AVLTree {
	 ...
	 * by the algorithm we saw in the class the w.c time complexity is O(log n).
	 */
	public AVLTree[] split(int x)
	{
		...
		if(prev == y.getLeft()) {
			...
			high.join(y, HighParent);

		}
		
		if(prev == y.getRight()) {
			AVLTree LowParent = new AVLTree(y.getLeft());
			low.join(y, LowParent); 
